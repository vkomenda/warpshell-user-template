...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    for (int i = 0; i<n_images; ++i) {
        FullyConnectedLayer(im+784*i,
                            reinterpret_cast<Vec_t const *>(weights1),
                            reinterpret_cast<Vec_t *>(data1),
                            bias+bias_offset[0], scales[0],
                            network_info[0], 1, network_info[1], 1, network_info[1], true);
#ifndef __SYNTHESIS__
        for (int i = 0; i<128; ++i) {
        ...
        FullyConnectedLayer(data1,
                            reinterpret_cast<Vec_t const *>(weights2),
                            reinterpret_cast<Vec_t *>(data2),
                            bias+bias_offset[1], scales[1],
                            network_info[1], 1, network_info[2], 1, network_info[2], true);
#ifndef __SYNTHESIS__
        for (int i = 0; i<256; ++i) {
        ...
        FullyConnectedLayer(data2,
                            reinterpret_cast<Vec_t const *>(weights3),
                            reinterpret_cast<Vec_t *>(data3),
                            bias+bias_offset[2], scales[2],
                            network_info[2], 1, network_info[3], 1, network_info[3], false);
#ifndef __SYNTHESIS__
        for (int i = 0; i<10; ++i) {
